  ...
  created() {
    ...
    this.$root.$on("addToCart", (id) => {
      ...
      if (localStorage.getItem("token")) {
        axios
          .get(`https://vuebackend.sehirulislamrehi.com/api/addtocart/${id}`)
          .then((res) => {
            ...
            this.cartsample.price = res.data.product.offer_price
              ...
              cart.filter((value, index) => {
                if( exist == false ){
                    if (value.id == this.cartsample.id) {
                        cart[index].qty += 1;
                        exist = true;
                    }
                }
